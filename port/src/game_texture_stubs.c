 ...
 * mincore.lib / onecore.lib — NOT kernel32.lib. Linking those brings
 * non-trivial runtime deps. Resolve at runtime via GetProcAddress so
 * the executable stays linked only against kernel32 + CRT, and we get
 * a clean fallback on Windows versions older than 10 1803. */
...
static NJS_TEXMEMLIST* tex_pool_alloc_low4g(size_t bytes) {
     ...
     * image base + heap usually start above 4 GiB, so the first few 256 MiB
     * slots below 2 GiB are almost always free. */
    if (!p) {
